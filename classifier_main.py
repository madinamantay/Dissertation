...
def main(args):
    match args.action:
        case 'train':
            train(args.train_dir, args.model_name, args.model_path, args.report_file)
        case 'test':
            test( args.model_name, args.model_path, args.test_dir)
        case 'test_once':
            test_once( args.model_name, args.model_path, args.image_path)
        case 'plot_metrics':
            plot_metrics(args.report_file, args.output_file)


def train(train_dir: str, model_name: str, model_path: str, report_file: str):
    cl = Classifier()

    data_sh, model_sh = _get_shapes(model_name)

    x, y = load_train_dataset(train_dir, dim=data_sh)
...
def test(model_name: str, model_path: str, test_dir: str):
    cl = Classifier()
...
def test_once(model_name: str, model_path: str, image_path: str):
    cl = Classifier()
...
def _get_classifier_model(model_name, shape):
    if model_name == '':
        return get_classifier_model(NUM_CLASSES, shape)

    if model_name == 'vgg11':
        return get_vgg_11(NUM_CLASSES, shape)

    if model_name == 'resnet18':
        return get_res_net_18(NUM_CLASSES, shape)

    if model_name == 'mobilenet_v2':
        return get_mobilenet_v2(NUM_CLASSES, shape)
